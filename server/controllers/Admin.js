...
export const adminform1 = async(req, res) => {
  ...
  try {
      ...
      //     }
      // });
  const [users, metadata] = await db.query("select * from u_form_details  left join forms_view on forms_view.id=u_form_details.id where Service_No =:status",
  {
...
// ON a.Civil_Qualification = cq.Id
      const [services, metadata] = await db.query("select * from u_form_details  left join forms_view on forms_view.id=u_form_details.id");
      //select DISTINCT * from tests_view left join u_form_details on tests_view.id=u_form_details.id
      res.json(services);
...
where:{
    ...
    Name:name
        ...
        const n=name.toJSON()
        const users= [n.Service_No,n.Adhar,n.Voter,n.PAN,n.ECHS,n.Img,n.Name];
